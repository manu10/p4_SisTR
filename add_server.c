    ...
    if ((fd = open (SERVER_FIFO, O_RDONLY)) == -1)
     ...
     while (1) {
         ...
         if (bytes_read == 0)
             continue;
         if (bytes_read > 0) {
             //Extraigo el path del fifo al cual debo responder,
             //Por convencion en esta comunicacion en cada mensaje se pasa
             //como primer parametro antes d eun ", \n"
